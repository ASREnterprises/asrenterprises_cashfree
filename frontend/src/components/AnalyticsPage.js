      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-white mb-1">Business Analytics</h1>
          ...
          <button
            ...
            disabled={refreshing}
            className="bg-purple-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-purple-700 transition flex items-center space-x-2 disabled:opacity-50"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-gray-800 rounded-xl p-5 border border-gray-700">
            ...
            <div className="text-2xl font-bold text-white">{stats.total_chats || 0}</div>
            <div className="text-gray-400 text-sm">Chat Sessions</div>
          </div>
          <div className="bg-gray-800 rounded-xl p-5 border border-gray-700">
            <Calculator className="w-6 h-6 text-blue-400 mb-2" />
            <div className="text-2xl font-bold text-white">{stats.total_calculations || 0}</div>
            <div className="text-gray-400 text-sm">Solar Calculations</div>
          </div>
          <div className="bg-gray-800 rounded-xl p-5 border border-gray-700">
            <TrendingUp className="w-6 h-6 text-yellow-400 mb-2" />
            <div className="text-2xl font-bold text-white">{stats.high_score_leads || 0}</div>
            <div className="text-gray-400 text-sm">High Score Leads</div>
          </div>
          <div className="bg-gray-800 rounded-xl p-5 border border-gray-700">
            <BarChart3 className="w-6 h-6 text-purple-400 mb-2" />
            <div className="text-2xl font-bold text-white">{stats.total_campaigns || 0}</div>
            <div className="text-gray-400 text-sm">Marketing Campaigns</div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
            ...
            {stats.leads_by_district && stats.leads_by_district.length > 0 ? (
              <div className="space-y-3 max-h-80 overflow-y-auto">
                {stats.leads_by_district.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between">
                    <span className="text-gray-300">{item._id || 'Unknown'}</span>
              ...
              <p className="text-gray-400 text-center py-8">No district data available</p>
            )}
          </div>
            ...
            {stats.leads_by_status && stats.leads_by_status.length > 0 ? (
              <div className="space-y-3">
                {stats.leads_by_status.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-center justify-between">
                      <span className="text-gray-300 capitalize">{item._id || 'Unknown'}</span>
                      <div className="flex items-center space-x-3">
                        <div className="w-32 bg-gray-700 rounded-full h-2">
                          <div 
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
            ...
            {stats.leads_by_property_type && stats.leads_by_property_type.length > 0 ? (
              ...
              <p className="text-gray-400 text-center py-8">No property type data available</p>
            )}
          </div>
          ...
          <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-gray-700 rounded-lg p-4 text-center">
                ...
                <div className="text-gray-400 text-sm">Last Month</div>
              </div>
              <div className="bg-gray-700 rounded-lg p-4 text-center">
              ...
              <div className="bg-gray-700 rounded-lg p-4 text-center">
                ...
                <div className="text-gray-400 text-sm">Avg Lead Score</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
          ...
          {stats.recent_leads && stats.recent_leads.length > 0 ? (
                ...
                <tbody>
                  {stats.recent_leads.map((lead, idx) => (
                    <tr key={idx} className="border-b border-gray-700 hover:bg-gray-750">
                      ...
                      <td className="py-3 px-4 text-gray-300">{lead.district || lead.location || '-'}</td>
                      <td className="py-3 px-4 text-gray-300 capitalize">{lead.property_type || '-'}</td>
                      <td className="py-3 px-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                          (lead.lead_score || 0) >= 80 ? 'bg-green-600 text-white' :
                          (lead.lead_score || 0) >= 60 ? 'bg-yellow-600 text-white' :
            ...
            <p className="text-gray-400 text-center py-8">No recent leads available</p>
          )}
        </div>
      </div>
    </div>
