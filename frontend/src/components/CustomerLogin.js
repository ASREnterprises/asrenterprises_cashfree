            ...
            {step === "mobile" && (
              <div data-testid="customer-login-form">
                ...
                <div className="relative mb-4">
                  ...
                  <input data-testid="customer-mobile-input" type="tel" value={mobile} onChange={e => { setMobile(e.target.value.replace(/\D/g, "").slice(0, 10)); setError(""); }} onKeyDown={e => e.key === "Enter" && handleSendOtp()} placeholder="Enter your mobile number" maxLength={10} className="w-full pl-24 pr-4 py-3.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0369A1] text-[#073B4C] text-base" />
                </div>

                {error && <div data-testid="customer-error" className="bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 text-sm mb-4">{error}</div>}

                <button data-testid="customer-send-otp-btn" onClick={handleSendOtp} disabled={loading || cleanMobile().length !== 10} className="w-full bg-gradient-to-r from-[#0369A1] to-[#0284C7] text-white py-3.5 rounded-xl font-bold text-base flex items-center justify-center gap-2 hover:shadow-lg transition disabled:opacity-50">
            ...
            {step === "otp" && (
              <div data-testid="customer-otp-form">
                ...
                <div className="flex gap-2 justify-center mb-6">
                  {otp.map((digit, idx) => (
                    <input data-testid={`customer-otp-${idx}`} key={idx} ref={el => (otpRefs.current[idx] = el)} type="tel" maxLength={1} value={digit} onChange={e => handleOtpChange(idx, e.target.value)} onKeyDown={e => handleOtpKeyDown(idx, e)} className="w-11 h-12 text-center text-xl font-bold border-2 border-slate-200 rounded-xl focus:outline-none focus:border-[#0369A1] focus:ring-2 focus:ring-[#0369A1]/20 text-[#073B4C] transition" />
                  ))}
                </div>
                ...
                <div className="flex items-center justify-between">
                  <button data-testid="customer-change-mobile" onClick={() => { setStep("mobile"); setOtp(["","","","","",""]); setError(""); setSuccess(""); }} className="text-slate-500 text-sm hover:text-[#073B4C] transition flex items-center gap-1">
                    ← Change mobile
                  </button>
